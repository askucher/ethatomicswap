...
contract AtomicSwap {
    ...
    function initiate(uint _refundTime, bytes20 _hashedSecret, address _participant) payable isNotInitiated(_hashedSecret) {
    ...
    function participate(uint _refundTime, bytes20 _hashedSecret, address _initiator) payable isNotInitiated(_hashedSecret) {
        swaps[_hashedSecret].refundTime = _refundTime;
        swaps[_hashedSecret].initTimestamp = block.timestamp;
    ...
    function redeem(bytes32 _secret, bytes20 _hashedSecret) isRedeemable(_hashedSecret, _secret) {
    ...
    function refund(bytes20 _hashedSecret) isRefundable(_hashedSecret) {
        if (swaps[_hashedSecret].state == State.Participant) {
            swaps[_hashedSecret].participant.transfer(swaps[_hashedSecret].value);
        }
        if (swaps[_hashedSecret].state == State.Initiator) {
